        ...
        do {
            ...
            DispatchQueue.main.async {
                ...
                if let data = UserDefaults.standard.object(forKey: OfflineDownload_KEY) as? [[String:Any]]{
                    var arrDownloadData = data
                    if data.count > 0{
                        for i in 0...data.count-1{
                            var donwloadData = data[i]
                            if donwloadData[FileURL_Key] as? String == downloadTask.originalRequest?.url?.absoluteString{
                                donwloadData[isDownloadComplete_Key] = true
                                arrDownloadData[i] = donwloadData
                                UserDefaults.standard.setValue(arrDownloadData, forKey: OfflineDownload_KEY)
                            }
                        }
                    }
                }
        ...
        if let data = UserDefaults.standard.object(forKey: OfflineDownload_KEY) as? [[String:Any]]{
            ...
            if data.count > 0{
                for i in 0...data.count-1{
                    var donwloadData = data[i]
                    if donwloadData[FileURL_Key] as? String == downloadTask.originalRequest?.url?.absoluteString{
                        donwloadData[progressData_Key] = progress
                        arrDownloadData[i] = donwloadData
                        UserDefaults.standard.setValue(arrDownloadData, forKey: OfflineDownload_KEY)
                    }
                }
